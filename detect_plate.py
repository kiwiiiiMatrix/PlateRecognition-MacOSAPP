...
def detect_Recognition_plate(model, orgimg, device,plate_rec_model,img_size,is_color=False):#获取车牌信息
    """
        ...
        参数：
        - model (object): 车牌检测模型。
        - orgimg (numpy.ndarray): 输入的原始图像。
        - device (str): 设备类型（如 'cpu' 或 'cuda'）。
        - plate_rec_model (object): 车牌识别模型。
        - img_size (int): 输入图像的尺寸。
        - is_color (bool): 是否进行颜色识别（默认值为 False）。

        返回：
        - dict_list (list): 包含检测结果的字典列表，每个字典包含车牌信息。
        """
    # Load model
...
def draw_result(orgimg,dict_list,is_color=False):
    """
       ...
       参数：
       - orgimg (numpy.ndarray): 输入的原始图像。
       - dict_list (list): 包含检测和识别结果的字典列表，每个字典包含车牌信息。
       - is_color (bool): 是否进行颜色识别（默认值为 False）。

       返回：
       - orgimg (numpy.ndarray): 绘制了车牌信息的图像。
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_path', type=str, default='F:\\pesonal_file\\projuect_for_sale\\ALPR-master\\save', help='source')     #图片路径
    ...
    parser.add_argument('--video', type=str, default='F:\\pesonal_file\\projuect_for_sale\\ALPR-master\\save\\test.mp4', help='source')                       #视频的路径
    ...
    if not opt.video:     #处理图片
        ...
        if capture.isOpened():
            while True:
                ...
                dict_list=detect_Recognition_plate(detect_model, img, device,plate_rec_model,opt.img_size,is_color=opt.is_color) # 这个就是需要的结果了 # TODO
                ori_img=draw_result(img,dict_list)
                t2 =cv2.getTickCount()
